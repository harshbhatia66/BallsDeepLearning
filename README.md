 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# Group name: BallsDeepLearning\n",
    "\n",
    "**Submitted on:** 06/08/2023\n",
    "\n",
    "| Harsh Bhatia   | h.bhatia@student.unsw.edu.au       | z5363556   |\n"
   ]
  },
  {
   "attachments": {},
   ...
   "source": [
    "# Table of Contents\n",
    "\n",
    "1. [Introduction to Modelling Problem](#introduction-to-modelling-problem)\n",
    "2. [Goal of Our Project](#goal-of-our-project)\n",
    "3. [Extending NLP-Powered Chatbot to Real Business Solutions and Beyond](#extending-nlp-powered-chatbot-to-real-business-solutions-and-beyond)\n",
    "4. [Choice of Model](#choice-of-model)\n",
    "5. [Data Deep Dive](#data-deep-dive)\n",
    "6. [Data Preparation](#data-preparation)\n",
    "7. [Exploring the Data](#exploring-the-data)\n",
    "8. [Input Data for Model](#Input-Data-Shape-and-Its-Use-in-DialoGPT)\n",
    "9. [DialoGPT - A GPT-2 Variant Model Architecture](#dialogpt---a-gpt-2-variant-model-architecture)\n",
    "10. [Training and Fine-Tuning DialoGPT](#training-and-fine-tuning-dialogpt)\n",
    "11. [Results](#results)\n",
    "12. [Fine-Tuning Parameters in DialoGPT](#fine-tuning-parameters-in-dialogpt)\n",
    "13. [Exploring Different Datasets and Models](#exploring-different-datasets-and-models)\n",
    "14. [Experiment with Philosophical Texts](#experiment-with-philosophical-texts)\n",
    "15. [Experiment with Shakespearean Texts](#experiment-with-shakespearean-texts)\n",
    "16. [Experiment with Pygmalion Model](#experiment-with-pygmalion-model)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "We were inspired by a similar project [(Rostyslav 2020)](https://towardsdatascience.com/make-your-own-rick-sanchez-bot-with-transformers-and-dialogpt-fine-tuning-f85e6d1f4e30), but with outdated code and references that could not be directly used for our purposes. We first started with replacing old features and libraries and restructuring the codebase. Through joint project work, we were able to build our own structure of preprocessing dataset, load/fine-tuning models, evaluating models, and obtain the final working chatbot. \n",
    "___\n",
    "\n",
    "## Goal of Our Project\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "### Summary Statistics\n",
    "The initial starting point of most EDAs is to determine the amounts of key statistical measures like the size of the dataset and number of distinct classes. This allows us to build a broad overview of the dataset we are working with and where we could explore it further. The following plots and figures helped in extracting and visualising said statistical measures from our dataset.\n",
    "\n",
    "For our Rick and Morty dataset, it contains 1905 total number of lines, spanning 3 seasons from the show and coming from 48 distinct characters. The character with the most frequent dialogue in the dataset is 'Rick' with\n",
    "420 total lines, as expected. An interesting feature of our dataset is that it contains many variations of the\n",
    "'Rick' character such as 'Pickle Rick' and 'Cop Rick', together these variations make up 182 lines of dialogue.\n",
    "This leaves the remaining 1303 lines of dialgoue to all non 'Rick' characters. From the character line distribution\n",
    "plot, we can observe that 'Rick' makes up less than a quarter of the total dialogue despite being the most frequent\n",
    "character. This suggests alongside the total number of lines in the dataset, that 'Rick's amount of responses might\n",
    "be too small or the dataset in general might be too small.\n",
    "\n",
    "In terms of 'Rick's line distribution by season, from the plot it can be observed that 'Rick' is consistent in his\n",
    "amount of dialogue by season, with a slight dip in season 2. This slight dip is largely made up with alternate 'Rick' characters.\n",
    "\n",
    "<img src=\"assets/character_line_frequency_pie_chart.png\" alt=\"1\" height=\"250\">\n",
    "<img src=\"assets/Rick_line_frequency_pie_chart_by_season.png\" alt=\"3\" height=\"250\">\n",
    "\n",
    "### Dialogue Analysis: Which Characters Talk The Most?\n",
    "\n",
    "In any dialogue-based dataset, one interesting aspect to explore is the distribution of dialogues among the characters. We can extend our character line frequency discussion from earlier to gain insights into the dynamics of the character interactions and the prominence of each character in the dataset.\n",
    "\n",
    ...
    "\n",
    "Below we have provided a graph to help us understand the character dialogue frequency. From this plot of the 15 most frequently speaking characters from the 48 total, it can be observed that the majority of the dialogue is sparsely split across the characters in the dataset, with almost all\n",
    "characters having less than 100 lines of dialogue. This can have the consequence during training of having our\n",
    "chatbot being trained with dialogue ranging with different sentiments which can improve performance since the training dialogue is split between many characters which likely will have very different styles.\n",
    "\n",
    ...
    "```\n",
    "\n",
    "In order to create n-grams from textual data, an important component is the choice of tokenizer used to generate the vocabulary of words from which frequencies are derived. Initially, when generating n-grams for the EDA, a weak tokenizer (I.E TreeBank Tokenizer from NTLK library) was used which tokenizes punctuations, and from this useful results concerning punctuation frequency for characters was derived. However, in order to build n-grams which capture frequencies of proper and useful words that appear in the dialogues of character, some pre-processing of the data is required along with a better tokenizer. In a second attempt, n-grams from our Rick and Morty dataset was generated by first pre-processing it to remove common english stopwords, common stopwords coming from the dataset itself (I.E this list of stowords were sourced from Kaggle workbook where the data was also sourced), converting all words to lowercase and also stripping off punctuation and numbers from character's dialogue. After pre-processing, a more sophisticated tokenizer was used (I.E Toktok tokenizer) which allowed us to generate useful n-grams for examining character's language in the dataset.\n",
    "\n",
    "Below is the python snippet of code that was used to pre-process a character's lines (I.E pd.Series) to remove unecessary details before the text is eventually passed to a tokenizer.\n",
    "\n",
    "```python\n",
    "import re\n",
    ...
    "import pandas as pd\n",
    "\n",
    "def process_character_line_series_for_tokenization(all_char_lines: pd.Series):\n",
    "    all_char_words_after_processing = []\n",
    "    # Common stopwords sourced from same source as dataset: https://www.kaggle.com/code/andradaolteanu/sentiment-analysis-rick-and-morty-scripts\n",
    "    rick_and_morty_dataset_stopwords = [\"thats\",\"weve\",\"hes\",\"theres\",\"ive\",\"im\",\n",
    "                                        \"will\",\"can\",\"cant\",\"dont\",\"youve\",\"us\",\n",
    "                                        \"youre\",\"youll\",\"theyre\",\"whats\",\"didnt\"]\n",
    "    \n",
    "    for line in all_char_lines:\n",
    "        for word in line.split(sep = ' '):\n",
    "            # Strip/skip whitespaces and removing numbers\n",
    "            if word == ' ' or re.search('\\d', word) is not None:\n",
    "                continue\n",
    "            # Remove punctuation\n",
    "            word = word.translate(str.maketrans('', '', string.punctuation + '’'))\n",
    "            # convert to lower case\n",
    "            word = word.lower()\n",
    "            # skip word if it is a common english stopword or common stopword in dataset\n",
    "            if word in stopwords.words('english') or word in rick_and_morty_dataset_stopwords:\n",
    "                continue\n",
    "            all_char_words_after_processing.append(word)\n",
    "    return ' '.join(all_char_words_after_processing)\n",
    "```\n",
    "\n",
    "These functions and additional pre-processing helped to analyze the frequency of unigrams, bigrams, and trigrams in our text data, which can provide insights into the most common words and phrases, the relationships between words, and the overall structure of the text. In the case of the Rick and Morty dataset, it was found that in the case of pre-processing the text and using the Toktok tokenizer, the character 'Rick' was found to have 3817 total word/token samples from his dialogue, with a vocabulary size of 1513. So, we can suggest 'Rick' is moderately varied with his dialogue.\n",
    "\n",
    "Below we have provided plots showing frequencies of the most common n-grams for the characters 'Rick', 'Morty', derived from pre-processing the character's lines before passing the data to a tokenizer. In the case of the character 'Rick', an additional plot is given, showing the frequencies of the common unigrams derived from his dialogue in the case of not pre-processing it before passing to a tokenizer.\n",
    "\n",
    "From the non pre-processed unigram plot for the character 'Rick', an interesting fact can be observed, which is 'Rick' likes to terminate his lines with exclamation marks very frequently. Overall, token '!' has a frequency of 140 and appears more frequently than common words like 'of' and 'and'. This suggests that we should expect the same frequent behavior with the text generated with our chatbot.\n",
    "\n",
    "From the processed unigram plots for the characters 'Rick' and 'Morty', it can be observed that both characters most frequently like to name each other in their dialogue. This suggests that 'Morty' is the most frequent conversational context for 'Rick', as we would expect, but more importantly that 'Rick' heavily re-introduces 'Morty' in his lines, since his frequency for 'Morty' far exceeds that of 'Morty' naming 'Rick' (I.E 239 compared to 88). This could have unintended negative consequences on the performance of the chatbot however, since the chatbot could learn to be conditioned to generate 'Morty' tokens even when the context doesn't permit it.\n",
    "\n",
    ...
    "<img src=\"assets/Rick_unigram_frequency_no_processing.png\" alt=\"4\" height=\"250\">\n",
    "<img src=\"assets/Rick_unigram_frequency_chart.png\" alt=\"5\" height=\"250\">\n",
    "<img src=\"assets/Rick_bigram_frequency_chart.png\" alt=\"6\" height=\"250\">\n",
    "<img src=\"assets/Rick_trigram_frequency_chart.png\" alt=\"7\" height=\"250\">"
   ]
  },
  {
   "attachments": {},
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "```\n",
    "Cross-Entropy Loss = - Σ (y_i * log(p(y_i)))\n",
    "```\n",
    "\n",
    "Where:\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    - `Σ` denotes the sum over all classes (words in the vocabulary)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        # 'load_state_dict' is a PyTorch method used to load a model’s (or optimizer’s/scheduler’s) saved parameters.\n",
    "\n",
